...
if (isset($_SESSION["id"]) && isset($_SESSION["role"]) && $_SESSION["role"] === 'guru') {
    ...
    function getAbsensi($conn, $tanggal, $kelas, $limit, $offset)
    {
        if ($kelas == '') {
            $stmt = $conn->prepare("
                SELECT * FROM absensi 
    ...
    if ($currentkelas == '') {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) AS total FROM absensi WHERE tanggal = ? AND kelas = ? AND status = 'Diproses'");
    ...
    $absensi = getAbsensi($conn, $tanggal, $currentkelas, $limit, $offset);
    ?>
                ...
                <div class="table-data">
                    <table>
                        ...
                        <tbody>
                            ...
                            $no = $offset + 1;
                            if (empty($absensi)) {
                                echo "<tr><td colspan='5' style='text-align:center;padding:10px;'>Tidak ada yang absen.</td></tr>";
                            } else {
